...
$resultAwards = &doQuery("
	SELECT
		".$db_prefix."hlstats_Awards.awardId,
		".$db_prefix."hlstats_Awards.game,
		".$db_prefix."hlstats_Awards.awardType,
		".$db_prefix."hlstats_Awards.code
	FROM
		".$db_prefix."hlstats_Awards
	LEFT JOIN ".$db_prefix."hlstats_Games ON
		".$db_prefix."hlstats_Games.code = ".$db_prefix."hlstats_Awards.game
	WHERE
		".$db_prefix."hlstats_Games.hidden='0'
	ORDER BY
		".$db_prefix."hlstats_Awards.game,
		".$db_prefix."hlstats_Awards.awardType
");
...
while( ($awardId, $game, $awardType, $code) = $resultAwards->fetchrow_array )
{
	...
	if ($awardType eq "O")
	{
		$table = "".$db_prefix."hlstats_Events_PlayerActions";
		$join  = "LEFT JOIN ".$db_prefix."hlstats_Actions ON ".$db_prefix."hlstats_Actions.id = $table.actionId";
		$matchfield = "".$db_prefix."hlstats_Actions.code";
		$playerfield = "$table.playerId";
	}
	elsif ($awardType eq "W")
	{
		$table = "".$db_prefix."hlstats_Events_Frags";
		$join  = "";
	...
	$result = &doQuery("
		...
		FROM
			$table
		LEFT JOIN ".$db_prefix."hlstats_Players ON
			".$db_prefix."hlstats_Players.playerId = $playerfield
		$join
		WHERE
			$table.eventTime < CURRENT_DATE()
			AND $table.eventTime > DATE_SUB(CURRENT_DATE(), INTERVAL $opt_numdays DAY)
			AND ".$db_prefix."hlstats_Players.game='$game'
			AND ".$db_prefix."hlstats_Players.hideranking='0'
			AND $matchfield='$code'
		GROUP BY
			$playerfield
		ORDER BY
			awardcount DESC,
			".$db_prefix."hlstats_Players.skill DESC
		LIMIT 1
	");
