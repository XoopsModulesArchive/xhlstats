...
sub recordEvent
{
	...
	my $query = "
		INSERT$insertType INTO
			".$db_prefix."hlstats_Events_$table
			(
				eventTime,
				serverId,
				map"
...
sub getPlayerId
{
	...
	my $query = "
		SELECT
			playerId
		FROM
			".$db_prefix."hlstats_PlayerUniqueIds
		WHERE
...
a MySQL database.
  ...
  -m, --mode=MODE                 player tracking mode (Normal, LAN or NameTrack)  [$g_mode]
      ...
      --db-prefix=PREFIX          table prefix  [$db_prefix]
...
;
	...
	if ($c % 500 == 0 && $g_deletedays)
	{
		...
		foreach $eventTable (keys(%g_eventTables))
		{
			if ($g_debug > 0)
			{
				print "-> ".$db_prefix."hlstats_Events_$eventTable ... "
			}

			&doQuery("
				DELETE$deleteType FROM
					".$db_prefix."hlstats_Events_$eventTable
				WHERE
